...
export function scrollToSectionWithOffset() {
    document.querySelectorAll('.navbar-link').forEach(link => {
        ...
        if (!href || href === "#") return; // Skip if href is empty or "#"

        const targetId = href.substring(1);

        // Exclude the 'portfolio' link from scroll behavior
        if (targetId === 'portfolio') return;

        link.addEventListener('click', function (event) {
            const targetElement = document.getElementById(targetId);

            // If no target element is found, skip
...
export function togglePortfolioActiveState() {
    const portfolioParent = document.querySelector('.portfolio-parent');

    document.addEventListener('click', (e) => {
        const isClickInside = portfolioParent.contains(e.target);

        if (isClickInside) {
            e.preventDefault();
            portfolioParent.classList.toggle('active');

            // If we just closed it, remove focus
            if (!portfolioParent.classList.contains('active')) {
                const link = portfolioParent.querySelector('.navbar-link');
                if (link) link.blur();
            }
        } else {
            portfolioParent.classList.remove('active');
        }
